...
class ValidationManager {
 ...
 isDuplicate(contact, currentContactId = null) {
   return this.contacts.some((existingContact) => {
     console.log( `the current contact id : ${contact.id}`)
...
class ContactManager {
 ...
 async handleFormSubmission() {
   ...
   const newContact = {
       id: contactId, // Include ID if editing
       full_name: document.getElementById("contact-full-name").value,
       email: document.getElementById("contact-email").value,
       phone_number: document.getElementById("contact-phone-number").value,
   };
   ...
   if (validation.validateContact(newContact, contactId).valid) {
       // Edit flow
       console.log(`Editing contact with ID: ${contactId}`);
       await this.editContact(newContact);
   } else if (validation.validateContact(newContact).valid) {
       // Add flow
       console.log("Adding new contact");
       await this.addContact(newContact);
   } else {
    this.displayError(validation.validateContact(newContact).message); 
    return; 
   }
